...
@Operation(
        summary = "Сохранение отправляемых api key'ев",
        security = @SecurityRequirement(name = API_KEY_SECURITY_NAME),
        responses = {
                @ApiResponse(
                        responseCode = "201",
                        description = "Api key'и сохранены",
                        content = {
                                @Content(mediaType = "application/json", schema = @Schema(implementation = OutgoingApiKeyResponse.class))
                        }
                ),
                @ApiResponse(
                        responseCode = "401",
...
public @interface ApiSaveOutgoing {
}